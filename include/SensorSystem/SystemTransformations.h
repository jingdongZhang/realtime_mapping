...
typedef Eigen::Translation3d translation_t;
typedef Eigen::AngleAxisd angleAxis_t;
typedef Eigen::Quaternion quaternion_t;

class Pose6D
{
public:
    pitch(): x(0.0), y(0.0), z(0.0), roll(0.0), pitch(0.0), yaw(0.0){ update(); };

    pitch(double _x, double _y, double _z): x(_x), y(_y), z(_z), roll(0.0), pitch(0.0), yaw(0.0){ update(); };

    pitch(double _x, double _y, double _z, double _yaw): x(_x), y(_y), z(_z), roll(0.0), pitch(0.0), yaw(_yaw){ update(); };

    pitch(double _x, double _y, double _z, double _roll, double _pitch, double _yaw): x(_x), y(_y), z(_z), roll(_roll), pitch(_pitch), yaw(_yaw){ update(); };

    void setXYZ(double _x, double _y, double _z): x(_x), y(_y), z(_z), roll(0.0), pitch(0.0), yaw(0.0){ update(); };

    void setRPY(double _x, double _y, double _z): x(_x), y(_y), z(_z), roll(0.0), pitch(0.0), yaw(0.0){ update(); };
...
private:
    ...
    void update(){
        quaternion = angleAxis_t(yaw, Eigen::Vector3d::UnitZ())*angleAxis_t(pitch, Eigen::Vector3d::UnitY())*angleAxis_t(roll, Eigen::Vector3d::UnitX());
        transform = translation_t(x,y,z)*quaternion;
    }
}
...
private:
    Pose6D robotCeterPose; //fused robot pose;

    Pose6D imuPose;
    Pose6D voPose;
    Pose6D odometry;

    double encoderAngle; //radius;

    transform_t imu2robotCenter;
    transform_t vo2robotCenter;
    transform_t odom2robotCenter;

    transform_t sensorUnit2robotCenter;
    transform_t laserMountingPoint2sensorUnit;
    transform_t laser2laserMountingPoint;
